...
export class DashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.jobData = resolvedData.data[0];
    ...
    if (!this.recipientService.welcomeTourNewUser) {
      this.recipientService.welcomeTourReset = this.authService.userData[
        "last_login"
      ]
        ? true
        : false;
  ...
  ngAfterViewInit() {
    this.carousel.slide.takeWhile(() => this.active).subscribe(slide => {
      this.recipientService.currentSlide = parseInt(slide.current, 0);
      if (
        this.recipientService.currentSlide === 0 &&
        slide.direction === NgbSlideEventDirection.LEFT
      ) {
